...
async function closeSession(guildId: string): Promise<string> {
  ...
  participants.forEach((p, i) => teams[i % session.teamCount].push(p));
...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName("close").setDescription("参加受付を締め切り、チームを発表します")
    ),
    ...
    if (sub === "start") {
      if (sessions.has(guildId)) {
        await interaction.reply({
          content: "すでにチーム分けが進行中です。`/team close` で締め切ってください。",
          ephemeral: true,
        });
        return;
      }
